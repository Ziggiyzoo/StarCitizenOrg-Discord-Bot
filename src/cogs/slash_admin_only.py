...
class SlashAdminOnly(commands.Cog):
    ...
    @commands.slash_command(
        name="add-guild", description="Used to set up the server, adding it to the DB."
    )
    async def add_guild(self, ctx, spectrum_id: discord.Option(str)):
        ...
        success = await database_connection.add_guild(
        ...
        if success:
            ...
            await ctx.response.send_message(
                resources_logic.get_string("ERROR_MESSAGES", "add_guild_error"),
                ephemeral=True,
            )
    ...
    @commands.slash_command(
        name="remove-guild",
        description="Used to remove the server, removing it to the DB.",
    )
    async def remove_guild(self, ctx):
        ...
        if success:
            ...
            await ctx.response.send_message(
                resources_logic.get_string("ERROR_MESSAGES", "remove_guild_error"),
                ephemeral=True,
            )
